...
#' @description Merges non-spatial and spatial model results into final output
#' @param prep list from STdiff_select_genes
#' @param spatial list from STdiff_fit_spatial containing spatial model results
...
STdiff_compile_results = function(prep=NULL, spatial=NULL){
  ...
  if(!is.null(spatial) && length(spatial) > 0 && !is.null(spatial[['sp_models']]) && length(spatial[['sp_models']]) > 0){
    ...
    for(sample_name in names(result_de)){
      if(pairwise){
        tibble_tmp = sp_de %>%
          ...
          dplyr::left_join(., sp_de %>%
                             ...
                             tibble::add_column(exp_adj_p_val=p.adjust(.[['exp_p_val']], method=prep[['pval_adj']])), by=c('sample', 'gene', 'cluster_1', 'cluster_2')) %>%
        ...
        tibble_tmp = sp_de %>%
          ...
          dplyr::left_join(., sp_de %>%
                             ...
                             dplyr::filter(sample == sample_name) %>%
                             dplyr::filter(!is.na(exp_p_val) & exp_p_val != -9999) %>%
                             tibble::add_column(exp_adj_p_val=p.adjust(.[['exp_p_val']], method=prep[['pval_adj']])), by=c('sample', 'gene', 'cluster_1')) %>%
          dplyr::relocate(comments_spatial, .after='exp_adj_p_val')
      ...
      result_de[[sample_name]] = result_de[[sample_name]] %>%
        dplyr::mutate(spatialmod=dplyr::case_when(is.na(exp_p_val) ~ '2', TRUE ~ '1'))
