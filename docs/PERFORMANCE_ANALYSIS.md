...
**Key Finding:** DynamicTreeCut (6.30 ms) + calculate_dist_matrices
(4.80 ms) = **80.1%** of total time
...
1.  **Vectorization**: R’s built-in functions are optimized for
    vector/matrix operations
    - Use [`dist()`](https://rdrr.io/r/stats/dist.html) on matrices
      instead of looping over genes
...
2.  **Memory Copy Avoidance**:
    ...
    - Clear large objects with [`rm()`](https://rdrr.io/r/base/rm.html)
      when done
3.  **Matrix Operations**:
    - Use [`crossprod()`](https://rdrr.io/r/base/crossprod.html) instead
      of `t(X) %*% X`
