...
Refactor `STclust` function into modular components while maintaining
backward compatibility through a
[`STclust_legacy()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust_legacy.md)
function.

**Status:** ✅ SThet refactored (commit 3fdd76e) **Target:** STclust
...
(lines 1-208) - Calculates spatially-informed clusters - Uses weighted
distance matrices (expression + spatial) - Supports two clustering
methods: - DTC (DynamicTreeCut) for adaptive k - Fixed k values for
range testing

**Helper functions (lines 210-350):** 1.
[`calculate_dist_matrices()`](https://acsoupir-oc.github.io/spatialGE/reference/calculate_dist_matrices.md) -
Calculate and scale distance matrices (35 lines) 2.
[`calculate_weighted_dist()`](https://acsoupir-oc.github.io/spatialGE/reference/calculate_weighted_dist.md) -
Create weighted distance matrices (24 lines) 3.
[`get_hier_clusters_dtc()`](https://acsoupir-oc.github.io/spatialGE/reference/get_hier_clusters_dtc.md) -
DTC-based clustering (45 lines) 4.
[`get_hier_clusters_ks()`](https://acsoupir-oc.github.io/spatialGE/reference/get_hier_clusters_ks.md) -
Fixed-k clustering (52 lines)

**Total:** ~156 lines of code (194 lines including documentation and
whitespace)
...
- **[`STclust_select_genes()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust_select_genes.md)** -
  Variable gene selection and count filtering
...
- **[`STclust()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust.md)** -
  Public wrapper maintaining original API

#### R/STclust_helpers.R (New)

- **[`calculate_dist_matrices()`](https://acsoupir-oc.github.io/spatialGE/reference/calculate_dist_matrices.md)** -
  Current helper (35 lines)
- **[`calculate_weighted_dist()`](https://acsoupir-oc.github.io/spatialGE/reference/calculate_weighted_dist.md)** -
  Current helper (24 lines)
- **[`get_hier_clusters_dtc()`](https://acsoupir-oc.github.io/spatialGE/reference/get_hier_clusters_dtc.md)** -
  DTC clustering (45 lines)
- **[`get_hier_clusters_ks()`](https://acsoupir-oc.github.io/spatialGE/reference/get_hier_clusters_ks.md)** -
  Fixed-k clustering (52 lines)
...
to
[`STclust_legacy()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust_legacy.md)
with: - Same signature and behavior - Exported for reproducibility -
...
If you need the original behavior for reproducibility, use
[`STclust_legacy()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust_legacy.md):
...
- Update
  [`STclust()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust.md)
  documentation with refactoring notes
- Add deprecation note for
  [`STclust_legacy()`](https://acsoupir-oc.github.io/spatialGE/reference/STclust_legacy.md)
  (optional, or keep as “legacy for reproducibility”)
- Document each new modular function

### Step 8: Commit and Push Strategy

**Commit 1: Baseline & Structure** - Run baseline tests - Create helper
functions file - Verify baseline still passes

**Commit 2: Core Modular Functions** - Add core modular functions - Test
each module independently

**Commit 3: Wrapper & Legacy** - Create STclust wrapper - Create
STclust_legacy - Add comparison test

**Commit 4: Documentation** - Update README.md - Update roxygen2 docs -
...
Basic clustering (ks=2:3, ws=0.025)
...
**Estimated new structure:** - `STclust.R`: ~50 lines (wrapper +
sourcing) - `STclust_core.R`: ~200 lines (modular functions) -
`STclust_helpers.R`: ~150 lines (helper functions) - `STclust_legacy.R`:
~350 lines (original)

**Total lines:** ~750 lines (increase due to modularization and
documentation)
...
**Low Risk:** - Helper functions are self-contained and well-tested by
existing tests - Modular functions can be tested independently - Legacy
function preserves original behavior

**Medium Risk:** - Wrapper function needs careful parameter passing -
Parallelization logic needs verification

**Mitigation:** - Extensive testing at each step - Comparison tests with
legacy implementation - Incremental commits for easy rollback
...
**Ready to proceed?** Let me know if you’d like to start implementation
or modify the plan.
