...
-   samples:
    ...
    tests. Numbers follow the order in `names(x@counts)`. Sample names
    are also allowed. If NULL, performs tests on all samples

-   annot:

    a column name in `x@spatial_meta` containing the groups/clusters to
...
-   pval_thr:
    ...
    cut-off of adjusted p-values to define differentially expressed
    genes from non-spatial linear models. A proportion of genes
    (`sp_topgenes`) under this cut-off will be applied the spatial
    models. Default=0.05
...
uses `spaMM::fitme` and is computationally expensive even on HPC
environments. To run the STdiff using the non-spatial approach (faster),
set `sp_topgenes=0`.
