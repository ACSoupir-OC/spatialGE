...
```{r set_net_check, include=FALSE}
# Set network check flag - will be updated if download succeeds
net_check <- TRUE
```

## Spatially-resolved expression of triple negative breast cancer tumor biopsies

To show the utility of some of the functions in `spatialGE`, we use the spatial transcriptomics data set from @thrane_2018. This data set includes melanoma biopsies from 4 patients. The data is provided as count matrices and spatial coordinate files, which are directly compatible with `spatialGE`.

Data for all the tissue slices are available at the Gene Expression Omnibus (GEO). For the purpose of this tutorial, we will use four samples from four patients. 

The GEO repositories can be accessed using the following links:

-   [ST_mel1_rep2](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM3058123)
-   [ST_mel2_rep1](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM3058124)
-   [ST_mel3_rep1](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM3058125)
-   [ST_mel4_rep2](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM3058126)

For convenience, the data is also available at our [spatialGE_Data GitHub repository](https://github.com/FridleyLab/spatialGE_Data). During this tutorial, we will pull the data from this repository. Nonetheless, users are encouraged to download the data and explore its contents in order to familiarize with the input formats accepted by `spatialGE`. For each sample directory, the file `*_counts.tsv` contains the gene expression counts, and the `*_mapping.tsv` file contains the spatial coordinates.
...
```{r data_download1}
# Use melanoma_thrane dataset (smaller, reliable download)
...
zip file, which will be unzipped into a sub-directory called `melanoma_thrane`:

```{r data_download2}
tryCatch({ # In case data is not available from network
  download.file(lk, destfile=paste0(visium_tmp, '/', 'melanoma_thrane.zip'), mode='wb')
  zip_tmp = list.files(visium_tmp, pattern='melanoma_thrane.zip$', full.names=TRUE)
  unzip(zipfile=zip_tmp, exdir=visium_tmp)
  net_check <<- TRUE
  message("Successfully downloaded melanoma_thrane dataset")
}, error = function(e) {
...
```{r extrenal_figs2, echo=FALSE, fig.align='center', eval=FALSE}
...
```{r data_fpaths, eval=TRUE}
# Get count and coordinate file paths
count_files <- list.files(paste0(visium_tmp, '/melanoma_thrane'), pattern='_counts.tsv$', full.names=TRUE)
mapping_files <- list.files(paste0(visium_tmp, '/melanoma_thrane'), pattern='_mapping.tsv$', full.names=TRUE)
```

The meta data can be accessed directly from the `spatialGE` package installed in the computer like so:

```{r data_fpaths2, eval=TRUE}
clin_file <- list.files(paste0(visium_tmp, '/melanoma_thrane'), full.names=TRUE, pattern='thrane_clinical.csv')
```

We can load the files into an STlist using this command:

```{r create_stlist, warning=F, eval=TRUE}
# Create STlist with count and coordinate files
melanoma <- STlist(rnacounts=count_files, spotcoords=mapping_files, cores=2)

# Add clinical metadata manually
clin <- read.csv(clin_file)
# Fix sample names to match STlist format
clin$sample_name <- paste0(clin$sampleID, '_counts.tsv')
clin$sampleID <- NULL
melanoma@sample_meta <- tibble::as_tibble(clin)
```

The `melanoma` object is an STlist that contains the count data, spot coordinates, and clinical meta data.

```{r call_stlist, eval=FALSE}
melanoma
```

As observed, by calling the `melanoma` object, information on the number of spots and genes per sample is displayed. For count statistics, the `summarize_STlist` function can be used:

```{r count_stats, eval=TRUE}
...
```{r count_dstr, eval=TRUE}
cp <- distribution_plots(melanoma, plot_type='violin', plot_meta='total_counts')
cp[['total_counts']]
```

Now, let us remove spots with low counts by keeping only those spots with at least 5000 counts. We will also restrict the data set to spots expressing at least 1000 genes. We also will remove a few spots that have abnormally large number of counts, as in ST_mel1_rep2. These criteria are not a rule, and samples in each study have to be carefully examined. For example, this criteria may be not enough to reduce the differences in counts, especially for ST_mel3_rep1 and ST_mel4_rep2.

We run this filter with the `filter_data` function:

```{r filter_chunk, eval=FALSE}
melanoma <- filter_data(melanoma, spot_minreads=5000, spot_mingenes=1000, spot_maxreads=150000)

cp2 <- distribution_plots(melanoma, plot_type='violin', plot_meta='total_counts')
cp2[['total_counts']]
```
...
```{r heatmap_chunk, fig.align='center', eval=FALSE}
hm_p <- pseudobulk_heatmap(melanoma, plot_meta='patient', hm_display_genes=30)
```
...
Let's produce a quilt plot for the genes *NDRG1* and *IGKC* (hypoxia and B-cell markers, respectively), for sample number 1 of patient 14 (`samples=ST_mel1_rep2`).

```{r genequilt_chunk, eval=FALSE}
...
```{r genequilt_chunk2, fig.align='center', eval=FALSE}
...
The function `gene_interpolation` performs kriging of gene expression via the `fields` package. We specify that kriging will be performed for two of the spatial samples (`samples=c('ST_mel1_rep2', 'ST_mel2_rep1')`):

```{r genekrige_chunk, eval=FALSE}
melanoma <- gene_interpolation(melanoma, 
                           genes=c('NDRG1', 'IGKC'),
                           samples=c('ST_mel1_rep2', 'ST_mel2_rep1'), cores=2)
```
...
```{r plotkrige_chunk1, fig.align='center', eval=FALSE}
...
```{r plot_tissue, eval=FALSE}
ip = plot_image(melanoma, samples='ST_mel1_rep2')

ip[['image_ST_mel1_rep2']]
```
...
```{r clustespots_chunk, warning=F, message=F, eval=FALSE}
...
```{r plotclustspots_chunk, fig.align='center', eval=FALSE}
...
```{r clinplot_chunk, fig.align='center', eval=FALSE}
...
```{r sphet_table, echo=F, eval=FALSE}
...
```{r load_simpkg, message=F, eval=FALSE}
library('rpart')
library('spatstat')
...
```{r tissue_sim_1, warning=F, message=F, eval=FALSE}
...
```{r tissue_sim_2, warning=F, fig.height=4, fig.width=6, eval=FALSE}
...
```{r tissue_sim_3, warning=F, eval=FALSE}
...
```{r tissue_sim_4, warning=F, fig.height=4, eval=FALSE}
...
```{r tissue_sim_5, warning=F, message=F, eval=FALSE}
